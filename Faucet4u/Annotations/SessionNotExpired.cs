    ...
    public class SessionNotExpired : ValidationAttribute
    {
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            try
            {
                ...
                {
                    string queryToExecute = "Select SessionExpiry from Users where SessionId = @SessionId";
                    dynamic queryResult = connectionObject.QueryFirstOrDefault(queryToExecute, new { SessionId = valueAsString });
                    if (queryResult != null)
                    {
                        int comparisonResult = DateTime.Compare(queryResult.SessionExpiry, DateTime.Now);
