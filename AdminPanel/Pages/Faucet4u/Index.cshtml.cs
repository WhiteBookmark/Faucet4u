        ...
        private static string TableData()
        {
            try
            {
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    ...
                    result[0] = connectionObject.Query("SELECT COUNT(*) AS Count FROM SupportTickets WHERE [Read] = 0");
                    result[1] = connectionObject.Query("SELECT COUNT(*) AS Count FROM SupportTicketsReply WHERE [Read] = 0");
                    result[2] = connectionObject.Query("SELECT COUNT(*) AS Count FROM OrderHistory WHERE Credit > 0");
                    result[3] = connectionObject.Query("SELECT COUNT(*) AS Count FROM Withdrawal WHERE [Paid] IS NULL");
