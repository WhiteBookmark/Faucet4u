...
{
    public class PTPBlacklistModel : PageModel
    {
        private static string TableData()
        {
            try
            {
                ...
                {
                    return JsonConvert.SerializeObject(connectionObject.Query("SELECT Keyword FROM PTPBlacklist"));
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult OnPostAddPTPBlacklist()
        {
            try
            {
                #region Session validation
                ...
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    if (connectionObject.Execute(@"BEGIN
                                                DECLARE @Keyword varchar(max) = @KeywordInput

                                                INSERT INTO PTPBlacklist(Keyword) VALUES(@Keyword)
                                                END", new { KeywordInput = Request.Form["keyword"] }) > 0)
                    {
        ...
        public IActionResult OnPostDeletePTPBlacklist()
        {
            try
            {
                #region Session validation
                ...
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    if (connectionObject.Execute(@"BEGIN
                                                    DECLARE @Keyword varchar(max) = @KeywordInput
                                                    DELETE FROM PTPBlacklist WHERE Keyword = @Keyword
                                                    END", new { KeywordInput = Request.Form["keyword"] }) > 0)
                    {
                        ViewData["Success"] = "Success";
                        ViewData["TableData"] = TableData();
