...
{
    public class MiningModel : PageModel
    {
        private static string TableData()
        {
            try
            {
                ...
                {
                    return JsonConvert.SerializeObject(connectionObject.Query("SELECT Percentage FROM Mining"));
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult OnPostUpdatePercentage()
        {
            try
            {
                #region Session validation
                ...
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    if (connectionObject.Execute(@"BEGIN
                                                DECLARE @Percentage int = @PercentageInput

                                                UPDATE Mining SET Percentage = @Percentage
                                                END", new { PercentageInput = Request.Form["percentage"] }) > 0)
                    {
