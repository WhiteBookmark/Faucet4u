    ...
    public class SettingsModel : PageModel
    {
        private static string TableData()
        {
            try
            {
                ...
                {
                    return JsonConvert.SerializeObject(connectionObject.Query("SELECT Name, Value FROM Settings"));
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult OnPostReplaceValue()
        {
            try
            {
                #region Session validation
                ...
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    ...
                    if (connectionObject.Execute("UPDATE Settings SET  Value = @Value WHERE Name = @Name", new { Name = Request.Form["name"], Value = Request.Form["value"] }) > 0)
                    {
