...
namespace AdminPanel.Pages.Faucet4u
{
    public class OrdersModel : PageModel
    {
        private static string TableData()
        {
            try
            {
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    ...
                    result[0] = connectionObject.Query("SELECT * FROM OrderHistory");
        ...
        public IActionResult OnPostApproveOrder()
        {
            try
            {
                #region Session validation
                ...
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    ...
                    if (connectionObject.Execute("UPDATE OrderHistory SET Status = 'Approved', Remarks = @Remarks WHERE Reference = @Reference", new { Remarks = Request.Form["remarks"], Reference = Request.Form["reference"] }) > 0)
                    {
        ...
        public IActionResult OnPostRejectOrder()
        {
            try
            {
                #region Session validation
...
BEGIN
UPDATE OrderHistory SET Status = 'Rejected', Remarks = @Remarks WHERE Reference = @Reference
DECLARE @Username varchar(12)
SELECT @Username = Username FROM OrderHistory WHERE Reference = @Reference
DECLARE @Price decimal(9,8)
SELECT @Price = Price FROM OrderHistory WHERE Reference = @Reference
UPDATE Users SET PurchaseBalance = PurchaseBalance + @Price WHERE Username = @Username
END", new { Remarks = Request.Form["remarks"], Reference = Request.Form["reference"] }) > 0)
                    {
                        ViewData["Success"] = "Success";
                        ViewData["TableData"] = TableData();
                        return Page();
