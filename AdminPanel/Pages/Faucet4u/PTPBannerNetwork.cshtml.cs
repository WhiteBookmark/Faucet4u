...
{
    public class PTPBannerNetworkModel : PageModel
    {
        private static string TableData()
        {
            try
            {
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    ...
                    result[0] = connectionObject.Query("SELECT * FROM PTPSquareBannerNetwork");
                    result[1] = connectionObject.Query("SELECT * FROM PTPBannerNetwork");
        ...
        public IActionResult OnPostAddPTPSquareBannerNetwork()
        {
            try
            {
                #region Session validation
                ...
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    if (connectionObject.Execute(@"BEGIN
                                                DECLARE @HTMLCode varchar(max) = @HTMLCodeInput
                                                DECLARE @MaxId int
                                                ...
                                                SET @MaxId = 1
                                                END
                                                INSERT INTO PTPSquareBannerNetwork(Id, HTMLCode) VALUES(@MaxId, @HTMLCodeInput)
                                                END", new { HTMLCodeInput = Request.Form["HTMLCode"] }) > 0)
                    {
        ...
        public IActionResult OnPostDeletePTPSquareBannerNetwork()
        {
            try
            {
                #region Session validation
                ...
                using (SqlConnection connectionObject = new SqlConnection(Other.faucet4uSQLConnectionString))
                {
                    if (connectionObject.Execute(@"BEGIN
                                                    ...
                                                    DELETE FROM PTPSquareBannerNetwork WHERE Id = @Id
                                                    UPDATE PTPSquareBannerNetwork SET Id = Id -1 WHERE Id > @Id
