        ...
        <mdb-col>
          <mdb-card>
            <mdb-card-body class="text-left">
              <p class="h4 text-center py-4">Resend confirmation email</p>
              ...
              <div class="text-center">
                ...
                <mdb-btn
                  outline="secondary"
                  v-on:click="sendResendEmailApi()"
                  type="button"
                  >Send <mdb-icon far icon="paper-plane"
                /></mdb-btn>
              </div>
            </mdb-card-body>
          </mdb-card>
  ...
  methods: {
    ...
    sendResendEmailApi: function() {
      ...
      this.$validator.validateAll().then(result => {
        if (result === true) {
          grecaptcha
            .execute(this.recaptchav3SiteKey, { action: "Resend_Email" })
            .then(token => {
              this.axios
                .put(process.env.VUE_APP_API_EmailConfirm, {
                  email: this.email,
